...
const DayMeal = ({day}) => {
    ...
    return (    
        ...
        <div className="flex justify-between">
          <div>
            <h1 className="text-darkgreen font-bold lg:text-xl md:text-lg sm:text-base text-sm">{day}</h1>
          ...
          <div className=" flex items-end lg:text-base md:text-base sm:text-sm text-xs">
            <button onClick={()=>setShow(!show)} className="bg-lightgreen text-white lg:px-8 md:px-6 sm:px-4 px-3 py-2 rounded-lg font-bold">
             {show? "Ẩn bớt": "Xem chi tiết"}
            </button>
          </div>
        </div>
        {show&&(<div className="grid lg:grid-cols-3 md:grid-cols-2 grid-cols-2 gap-4">
          <Meal />
          <Meal />
          <Meal />
        </div>)}
